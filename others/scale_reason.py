# python scale_reason.py --dataset_name aime --problem_id 60-89 --repeat_id 3 --model_name Qwen/QwQ-32B --output_dir results/scale_reason_test --use_role_prompts
...
def generate_full_reasoning(problem, options=None, max_tokens=8192, role_type=None):
    """使用模型一次性生成完整的推理过程"""
    global model
    
    start_time = time.time()  # 记录开始时间
    
    prompt = get_first_user_msg(problem, options, role_type)
        ...
        if args.use_role_prompts:
            ...
            for role_type in [1, 2, 3]:
                ...
                if result["stop_reason"] == "finished":
                    result["answer"] = extract_final_answer(reasoning, args.dataset_name)
            ...
            # 不使用角色提示，但仍然生成三次，以保持一致性
            results = []
            for run_id in range(3):  # 生成三次
                logging.info(f"生成第 {run_id+1}/3 次推理...")
                reasoning, finished, num_tokens, elapsed_time, token_time = generate_full_reasoning(
                    problem, 
                    options=options, 
                    max_tokens=args.token_budget
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="results/scale_reason_test", 
