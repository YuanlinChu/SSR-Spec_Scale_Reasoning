...
for problem_id in problem_ids:
    ...
    if args.dataset_name == "aime":
        problem = args.dataset["problem"][problem_id - 60]
        options = None
    ...
    output_filename = os.path.join(output_dir, f"{problem_id}-{args.repeat_id}")
    
    if os.path.exists(f"{output_filename}.pickle"):
        logging.info(f"问题 {problem_id} 重复 {args.repeat_id} 已解决，跳过")
        completed_tasks += 1
        continue
