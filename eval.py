# python eval.py --dataset_name aime --model_size 32b
...
def evaluate_model(args):
    ...
    for filename in os.listdir(model_dir):
        ...
        try:
            ...
            # 检查stop_reason，如果是budget则直接算错误
            if reasoning_data.get("stop_reason") == "budget":
                is_correct = False
                logging.info(f"问题 {problem_id}（索引 {problem_idx}），重复 {repeat_id}: 由于达到token预算上限，直接判定为错误")
            else:
                # 只有当stop_reason为finished时才提取答案
