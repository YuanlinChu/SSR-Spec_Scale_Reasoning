...
parser.add_argument("--score_method", type=str, choices=["greedy", "average"], default="greedy",
                    help="Scoring method")
parser.add_argument("--output_dir", type=str, default="my_results/test", 
...
try:
    while True:
        ...
        # 1.5 使用小模型对自己的步骤进行评分
        small_model_score, small_model_justification, small_model_response, small_model_scoring_time = get_score(args, problem, steps_so_far + [step_str], model_size="1.5b", options=options)
        logging.info(f"[Step {step_id}] Small model self-score: {small_model_score}, Time: {small_model_scoring_time:.2f}s")
        total_small_model_time += small_model_scoring_time  # 将小模型评分时间也计入小模型总时间
        ...
        if score is not None and score >= args.score_threshold:
            logging.info(f"[Step {step_id}] Large model score {score}, accepted! Time: {small_model_time:.2f}s, Token time: {small_token_time:.2f}ms/token")
            base_model_step, num_output_tokens_base, large_model_time, large_token_time = None, None, 0, 0
        else:
            logging.info(f"[Step {step_id}] Large model score {score} rejected, falling back to base model")
        ...
        metadata = {
            ...
            "small_model_score": small_model_score,  # 添加小模型自评分
            "small_model_justification": small_model_justification,  # 添加小模型评分理由
            "large_model_score": score,  # 重命名为large_model_score以区分
            ...
            "small_model_scoring_time": small_model_scoring_time,  # 添加小模型评分时间
...
# 计算大小模型评分的相关性
small_model_scores = [m.get("small_model_score") for m in metadata_list if m.get("small_model_score") is not None]
large_model_scores = [m.get("large_model_score") for m in metadata_list if m.get("large_model_score") is not None]

# 确保两个列表长度相同
min_len = min(len(small_model_scores), len(large_model_scores))
if min_len > 1:  # 至少需要两个数据点才能计算相关性
    small_model_scores = small_model_scores[:min_len]
    large_model_scores = large_model_scores[:min_len]
    
    # 计算皮尔逊相关系数
    try:
        correlation = np.corrcoef(small_model_scores, large_model_scores)[0, 1]
        logging.info(f"大小模型评分的皮尔逊相关系数: {correlation:.4f}")
        
        # 添加到时间统计信息中
        time_stats["score_correlation"] = {
            "small_model_scores": small_model_scores,
            "large_model_scores": large_model_scores,
            "pearson_correlation": correlation
        }
    except Exception as e:
        logging.error(f"计算相关系数时出错: {e}")
else:
    logging.warning("数据点不足，无法计算相关系数")

# 添加评分相关信息
if "score_correlation" in time_stats:
    logging.info("评分相关性分析:")
    logging.info(f"  - 小模型评分: {time_stats['score_correlation']['small_model_scores']}")
    logging.info(f"  - 大模型评分: {time_stats['score_correlation']['large_model_scores']}")
    logging.info(f"  - 皮尔逊相关系数: {time_stats['score_correlation']['pearson_correlation']:.4f}")
logging.info("=" * 50)
